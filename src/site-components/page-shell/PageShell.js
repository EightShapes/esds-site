...
export class EsdsSitePageShell extends Slotify(LitElement) {
    ...
    selectAnimatableHeaderElements() {
        return {
            ...
            headerInner: this.querySelector(
            ...
            toggle: {
                callback: {
                    visible: (scrollingHeader) => {
                        const position = scrollingHeader.element.getBoundingClientRect()
                            .y
                        if (
                            position <= 0 &&
                            position >= -this.collapsedHeaderHeight
                        ) {
                            if (position < 0) {
                                headerInner.style.position = 'fixed'
                            } else {
